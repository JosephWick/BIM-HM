...
function out = run(b)
  ...
  ss.e12p_plate = ss.Vpl_scalar.*getE12(x2p(1),x3p(1), n_scalar, w);
  ...
  mat2np(Y, 'pickles/BP1vHM_Y.pkl', 'float64');
  ...
  mat2np(y.V, 'pickles/BP1vHM_V.pkl', 'float64');
  ...
  saveas(gcf, 'figures/BP1vHM_slip.png')
  ...
  xlabel('Time Steps')
  ylabel('Block')
  saveas(gcf, 'figures/BP1vHM_strainCenter.png')
  ...
  if Smovie
    ...
    clf;
    fig = figure;
    fname = 'figures/BP1vHM_strain.gif';
    for idx = 1:size(Epall, 2)
      oneE = Epall(:,idx);
      oneEsq = reshape(oneE, [ss.Ny, ss.Nz]);
      imagesc(oneEsq'); colormap(hot); colorbar; caxis auto;
      title(idx)
      drawnow
      frame = getframe(fig);
